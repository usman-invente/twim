  ...
  <head>
                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            <h5 class="font-weight-bold">{{ $pageTitle ?? trans('messages.location_category') }}</h5>
                            @if($auth_user->can('category add'))
                            <a href="{{ route('location-category.create') }}" class="float-right mr-1 btn btn-sm btn-primary"><i class="fa fa-plus-circle"></i> {{ trans('messages.add_form_title',['form' => trans('messages.location_category')  ]) }}</a>
                            @endif
                        </div>
                    </div>
                </div>
    ...
    <div class="card">
        ...
        <div class="row justify-content-between ">
            <div>
                <div class="col-md-12">
                  <form action="{{ route('location-category.bulk-action') }}" id="quick-action-form" class="form-disabled d-flex gap-3 align-items-center">
                    @csrf
                ...
                data--submit="{{ route('location-category.bulk-action') }}"
    ...
    <script>
        ...
        window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                  "type"   : "GET",
                  "url"    : '{{ route("location-category.index_data") }}',
                  "data"   : function( d ) {
                    d.search = {
                      value: $('.dt-search').val()
                    };
